...
while true; do
    if test $i -ge 100; then
        ...
        i=18
        if [ -z "$session" ]; then
            log "Login Fail ! session is empty . sleep for an hour !"
            i=100
            sleep 3600
            uid=$uid_orig
            continue
        else
            log "Login Successed ! session is $session"
...
#log "renew xunlei"
    ...
    if test $do_up_accel -eq 1; then
        ...
        if [[ -z $ret ]]; then
			log "up upgrade expired ! upgrade again after 1 min"
            sleep 60
            i=18
            continue
        fi
	...
	log "keepalive check pass ! current i = $i , after i++ then sleep 590s"
