...
day_of_month_orig=$fastd1ck_orig_day_of_month
orig_day_of_month=$fastd1ck_day_of_month_orig
...
upgrade (){
	...
	if test $do_down_accel -eq 1; then
		downResult=`$HTTP_REQ "$api_url/upgrade?peerid=$peerid&userid=$uid&sessionid=$session&user_type=1&client_type=android-swjsq-'''+APP_VERSION+'''&time_and=$_ts&client_version=androidswjsq-'''+APP_VERSION+'''&os=android-'''+OS_VERSION+'.'+OS_API_LEVEL+DEVICE_MODEL+'''&dial_account='''+dial_account+'''"`
		log "downResult is $downResult"
	fi
	if test $do_up_accel -eq 1; then
		upResult=$HTTP_REQ "$api_up_url/upgrade?peerid=$peerid&userid=$uid&sessionid=$session&user_type=1&client_type=android-uplink-'''+APP_VERSION+'''&time_and=$_ts&client_version=androiduplink-'''+APP_VERSION+'''&os=android-'''+OS_VERSION+'.'+OS_API_LEVEL+DEVICE_MODEL+'''&dial_account='''+dial_account+'''"
        log "UpResult is $upResult"
	fi
	dbus set fastd1ck_currentI=1
	exit
}
...
login (){
	...
	if [ ! -z "$loginkey" ]; then
		...
		if [ -z "$session" ]; then
			...
			dbus set fastd1ck_session=$session
		fi
	fi
	...
	dbus set fastd1ck_loginkey=$loginkey
	log "LoginKey is $loginkey"
	#i=18
}
...
i=$fastd1ck_currentI
session=$fastd1ck_session
loginkey=$fastd1ck_loginkey
